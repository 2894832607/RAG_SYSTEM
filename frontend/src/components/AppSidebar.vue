<template>
  <aside class="sidebar">
    ...
    <!-- Recent Chats (real chat history) -->
    <div class="sidebar-section" v-if="recentChats.length > 0">
      <div class="section-label">近期对话</div>
      <div class="recent-list">
        <div
          v-for="chat in recentChats"
          :key="chat.id"
          class="recent-item"
          @click="router.push(`/conversation/${chat.sessionId}`)"
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="flex-shrink:0;opacity:.5"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>
          <span class="recent-text">{{ truncate(chat.userMessage, 18) }}</span>
        </div>
      </div>
    </div>
...
interface RecentChat { id: number; userMessage: string; sessionId: string }
const recentChats = ref<RecentChat[]>([])

onMounted(async () => {
  try {
    const res = await fetchChatHistory(1, 15)
    recentChats.value = (res.data?.data?.items || []).map((item: any) => ({
      id: item.id,
