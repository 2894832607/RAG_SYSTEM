 ...
 */
export function fetchChatStream(message, sessionId, token) {
 ...
 * @param token  JWT token（Bearer）
 */
export function fetchStoryboardStream(sourceText, token) {
...
export async function fetchModelConfig() {
    ...
    if (!res.ok)
        throw new Error(`模型配置查询失败 ${res.status}`);
    return res.json();
}
/** 切换 LLM 配置 */
...
//# sourceMappingURL=poetryService.js.map