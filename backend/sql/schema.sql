...
-- ============================================================
-- 5. 对话消息历史表 (sys_chat_message)
--    用途: 持久化每一轮对话（用户发言 + AI回复），按账号隔离
-- ============================================================
CREATE TABLE IF NOT EXISTS sys_chat_message (
    id              BIGINT          PRIMARY KEY AUTO_INCREMENT          COMMENT '主键ID',
    user_id         BIGINT          NOT NULL                            COMMENT '用户ID，关联 sys_user.id',
    session_id      VARCHAR(64)     NOT NULL DEFAULT ''                 COMMENT '会话ID（UUID），同一会话的多轮对话共享',
    user_message    TEXT            NOT NULL                            COMMENT '用户发出的消息',
    ai_reply        MEDIUMTEXT      DEFAULT NULL                        COMMENT 'AI 回复完整文本（token 拼接）',
    gmt_create      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP  COMMENT '对话时间',
